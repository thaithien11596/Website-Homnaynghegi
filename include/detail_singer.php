...
<body>
	<?php 
		include('header1.php');
			...
			if(isset($_GET['idsinger']) && isset($_GET['idQg'])){
								$idsinger = $_GET['idsinger'];
								$idQg = $_GET['idQg'];

							}  
			$music=new Music();
			$data1=$music->randMusic($idQg);
			$detailSinger = $music->detailSinger($idsinger);
			$detailsingSinger = $music->detailsingSinger($idsinger);
			$randSinger = $music->randSinger($idQg);
		...
		<div class="container container3">
			<div class="row rowlbh" style="height: auto">
				<div class="col-sm-8 col8sing" style="height: auto">
						<?php 
								...
								foreach ($detailSinger as $result) {
								 		// echo '<pre>';
									// print_r($result);
										// if($result['idChitietbaihat'] = $result['idCasi'] ){
										?>
										    <label for="" style="color:white; font-size: 20px; color: #dedede;margin-top: 20px;">Tên ca sĩ: <?= $result['HotenCasi'] ?><a href="#" style="color: #dedede"></a></label>
                                           ...
                                           <!--  <div id="amazingaudioplayer-1" style="display:block;position:relative;width:100%;max-width:770px;height:auto; right: 3px; ">
                                            ...
                                            <!-- End of body section HTML codes -->
													...
													<div class="jumbotron jbstr">
														<ul>
															<li><img src="../img/<?= $result['imageCasi'] ?>" alt="" style= "width:125px;height:125px;border: 1px solid gray;box-shadow: 0 0 1px 1px gray"></li>
															<li style="margin:20px 10px 20px 10px"><p >Ngày sinh: <?= $result['NgaysinhCasi'] ?></p></li>
															<li style="margin:20px 10px 20px 10px"><p >Quê quán: <?= $result['QuequanCasi'] ?></p></li>
														</ul>
														
														
														
														<h4 class="display-3" style="float: left;color:#dedede;line-height: 1.5;text-align: justify; ">Tiểu sử: <?= $result['TieusuCasi'] ?></h4>
			...
			<div class="row" style="min-height: 250px">
				<div class="col-sm-12 colsing" style="right: 15px">
					<p style="margin-left: 15px;margin-top: 10px;margin-bottom: 0px;color: white;font-size: 22px">Ca sĩ cùng quốc gia:</p>
					<?php foreach ($randSinger as $result): ?>
													...
													<a href="detail_singer.php?idsinger=<?= $result['idCasi'] ?>&&idQg=<?= $result['idQuocgia'] ?>">
														<div class="card">
															...
															<div class="card-block">
																<h4 class="card-title" style="color: white;margin-top: 3px;margin-bottom: 3px"><?= $result['HotenCasi'] ?></h4>
															</div>
														</div>
													</a>
													</div>
			...
			<div class="col-sm-4 noidungtrai1" style="left:23px;top: 52px;border: 0px;height: auto;box-shadow: 0 0 0 0!important">
				<h3 >Bài hát cùng ca sĩ</h3>
		 		<div class="list-group">
		 			<?php foreach ($detailsingSinger as $result): ?>
