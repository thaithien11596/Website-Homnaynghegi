...
<?php
		...
		$data=$music->demgetNhacsi($idQg);
		$data1=$indexMusic->indexRandMusic();
		$demNhacsi = count($data);
...
?>
						<div class="noidungbh">
				<div class="baihatnb">
					<p style="color:white">Nhạc sĩ <?= $result['TenQuocgia'] ?><span class="glyphicon glyphicon-menu-right menuright"></span></p>
					...
					<div class="row row1">
						<div class="col-sm-8 colbhnoibat">
								...
								<?php 
										...
										if(isset($_GET['trangNhacsi'])){
											 $i = $_GET['trangNhacsi'];
											 $sl = ($i - 1)*12;
											$music=new Music();
											$data=$music->getNhacsi($sl,$idQg);
													...
													<a href="detail_Nhacsi.php?idNhacsi=<?= $result['idNhacsi'] ?>&&idQg=<?= $result['idQuocgia'] ?>">
														<div class="card">
															...
															<div class="card-block">
																<h4 class="card-title"><?= $result['HotenNhacsi'] ?></h4>
															</div>
														</div>
													</a>
													</div>
						...
						<?php  for($i=1;$i<=ceil($demNhacsi/12);$i++) { ?>
						<a href="nhacsiForm.php?idQg=<?= $idQg ?>&&trangNhacsi=<?= $i ?>" style="color:white;background-color: black;padding: 10px;"><?= $i ?></a>
