...
<?php 
  ...
  $sql="select * from nhacsi limit $trang1,5"; // trang1 (vị trí nhạc sĩ hiện tại) , lấy 5 nhạc sĩ
  ...
  $matk=$_POST['timkiem'];
  $sql_tk="select * from nhacsi where idNhacsi='$matk'";
  ...
  $datatk = $objStmtk->fetchAll(PDO::FETCH_ASSOC);
      ...
      $m=$rowtk['idNhacsi'];
      if($matk=='$m'){
        echo '<script>alert("Không tìm thấy Id nhạc sĩ")</script>';
      }else{
  ?>
...
<form action="" method="post" style="float:right">
    Nhập ID Nhạc sĩ:<input type="text" name="timkiem" />
    <input type="submit" name="sub-timkiem" value="Tìm Kiếm Nhạc Sĩ" />
</form>
